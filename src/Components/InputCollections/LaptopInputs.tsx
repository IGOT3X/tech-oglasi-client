...
const LaptopInputs = ({ setspecifications, specifications }: { setspecifications: Function, specifications: TSpecifications }) => {
    ...
    return (
        <div className="flex flex-wrap self-start w-full pt-[24px] gap-[24px]">
            ...
            {!specificationsChosen?.screen &&
                <button className="h-[40px] px-[16px] rounded-[10px] flex items-center justify-items-center bg-transparent border border-mint gap-[8px] border-opacity-50" onClick={() => setSpecificationsChosen(prevState => {
                    return (
                        {
                            ...prevState,
                            screen: true
                    ...
                    <img className="w-[18px]" src="plus.svg" alt="" />
                    <p>Rezoucija ekrana</p>
                </button>
            }
            {specificationsChosen.screen &&
                <div className="flex flex-col gap-4 w-full animate__animated animate__fadeIn mt-[40px]">
                    <p className="text-center">Rezolucija ekrana</p>
                    <input className="px-2 h-[40px] bg-transparent border border-mint focus:outline-none w-full rounded-[10px]" type="text" placeholder="Rezolucija ekrana" value={specifications?.screenResolution} onChange={(e) => setspecifications((prevState: TSpecifications) => {
                        return (
                            {
                                ...prevState,
            ...
            {!specificationsChosen?.battery &&
                <button className="h-[40px] px-[16px] rounded-[10px] flex items-center justify-items-center bg-transparent border border-mint gap-[8px] border-opacity-50" onClick={() => setSpecificationsChosen(prevState => {
                    ...
                    <img className="w-[18px]" src="plus.svg" alt="" />
                    <p>Veličina baterije</p>
                </button>
            }
            {specificationsChosen.battery &&
                <div className="flex flex-col gap-4 w-full animate__animated animate__fadeIn mt-[40px]">
                    <p className="text-center">Veličina baterije</p>
                    <div className="px-2 h-[40px] bg-transparent border border-mint focus:outline-none w-full rounded-[10px] flex items-center justify-between">
                        <input className="h-[40px] bg-transparent focus:outline-none w-full rounded-[10px]" type="number" placeholder="Veličina baterije" value={specifications?.batterySize} onChange={(e) => setspecifications((prevState: TSpecifications) => {
                            return (
                                {
                                    ...prevState,
                                    batterySize: e.target.value
                                }
                            )
                        })} />
                        <p className="opacity-50">mA</p>
                    </div>
                </div>}
        </div>
    )
